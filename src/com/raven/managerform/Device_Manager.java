...
public class Device_Manager extends javax.swing.JPanel {
    public void loadForm4(){
            DefaultTableModel model = (DefaultTableModel) table.getModel();
            model.setRowCount(0); // Xóa dữ liệu cũ

            ArrayList<DeviceClass> devices = Device.SelectDevice(); // Gọi hàm từ lớp Area

            for (DeviceClass device : devices) {
                String matb = device.getMatb();
                String tentb = device.getTentb();
                String maltb = device.getLoaitb();
                String trangthai = device.getTrangthai();
                String ngaymua = device.getCreatedAt().toString();

                // Xử lý enum StatusType nếu có
                switch (trangthai) {
                    case "FIXING":
                        model.addRow(new Object[]{matb, tentb, maltb, StatusType.FIXING, ngaymua});
                        break;
                    case "PAUSING":
                        model.addRow(new Object[]{matb, tentb, maltb, StatusType.PAUSING, ngaymua});
                        break;
                    case "RUNNING":
                        model.addRow(new Object[]{matb, tentb, maltb, StatusType.RUNNING, ngaymua});
                        break;
                    default:
                        model.addRow(new Object[]{matb, tentb, maltb, trangthai, ngaymua});
                        break;
                }
            }
    ...
    private void initComponents() {
        ...
        table.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Mã TB", "Tên TB", "Loại TB", "Trạng Thái", "Ngày Mua"
            }
        ) {
    ...
    private void btnDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteActionPerformed
        DeleteUtil.deleteRecord(table, "THIET_BI", "MATB");
        loadForm4();
    ...
    private void btnUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnUpdateActionPerformed
        ...
        device.setMatb(table.getValueAt(selectedRow, 0).toString());
        device.setTentb(table.getValueAt(selectedRow, 1).toString());
        device.setLoaitb(table.getValueAt(selectedRow, 2).toString());
        device.setTrangthai(table.getValueAt(selectedRow, 3).toString());
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        try {
            device.setCreatedAt(sdf.parse(table.getValueAt(selectedRow, 4).toString()));
        } catch (ParseException ex) {
            Logger.getLogger(Device_Manager.class.getName()).log(Level.SEVERE, null, ex);
        }

        // Mở form cập nhật tổng quát
        Update updateForm = new Update((UpdatableEntity) device, this::loadForm4); // constructor mới
